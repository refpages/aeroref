...
We say that a system is **controllable** if eigenvalue placement is possible. An equivalent viewpoint is **finite-time reachability**: the system is controllable if, for any initial state $x(0)=x_0$ and any desired state $x_\mathrm{des}$, there exists a finite time $T>0$ and an input $u(t)$ on $[0,T]$ such that $x(T)=x_\mathrm{des}$.

We have seen eigenvalue placement with Ackermann's method (for the special case when $n_u = 1$) is only possible when the controllability matrix $W$ is invertible. Here is a generalization of that same result to any system:


<Result title="Controllability">
The system
