...
Other people use other symbols for both the variables and constants in a state space model. Indeed, we will sometimes use other symbols for these things as well. Here is an example:

$$
\dot{z} = Ez + Fv
$$

This linear system is also a "state space model," in which the state is $z$ and the input is $v$.
